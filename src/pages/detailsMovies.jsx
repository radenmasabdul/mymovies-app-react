...
const DetailsMovies = (props) => {
  ...
  useEffect (() => {
    ...
    return (
      <Container>
        <div className="max-w-7xl mx-auto mt-8 bg-gradient-to-l from-blue-700">
          <div className="flex flex-wrap ">
            <div className="w-full md:w-1/2 px-20">
              <img src={`https://image.tmdb.org/t/p/w500${data?.poster_path}`} alt={props.title} />
            </div>

            <div className="w-full md:w-1/2 p-4 border-dashed border-2 border-black dark:text-white dark:border-color-white">
              <p className="font-bold text-4xl max-md mx-auto border-b-2 border-blue-600  dark:border-color-white">{data?.original_title}</p>
